...
- **完成通知** —— 任务完成与长回复提醒，三层投递：应用内系统通知（SSE）、Web Push（VAPID）、以及应用关闭时也能送达的第三方渠道（PushPlus / Server酱 / Bark / Telegram）；阈值与间隔可在手机端配置。**分类型门控**（规划完成 / 后台任务 / 长回复）可独立开关，后台任务与长回复默认安静
...
- **第三方渠道** —— PushPlus（微信，国内用户推荐首选）、Server酱（微信）、Bark（iOS）、Telegram webhook，应用完全关闭也能送达；凭据只存宿主端，不经过设置面
...
| L3 | PushPlus / Server酱 / Bark / Telegram webhook | 应用关闭、浏览器已退出 | 凭据只存宿主端 |
...
**L3（第三方）。** PushPlus（微信）、Server酱（微信）、Bark（iOS）、Telegram webhook 在应用完全关闭、浏览器退出后仍能送达。凭据只存宿主端，设置面只显示各渠道是否已配置。
