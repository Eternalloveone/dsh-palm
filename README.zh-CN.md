...
- **完成通知** —— 任务完成与长回复提醒，三层投递：应用内系统通知（SSE）、Web Push（VAPID）、以及应用关闭时也能送达的第三方渠道（WxPusher / Server酱 / Bark / Telegram，另有降级保留的 PushPlus）；阈值与间隔可在手机端配置。**分类型门控**（规划完成 / 后台任务 / 长回复）可独立开关，后台任务与长回复默认安静
- **逐提供方用量与余额** —— 桌面端配置的每个模型提供方一张卡片：周用量进度条（Ollama Cloud）、账户余额（DeepSeek、Moonshot/Kimi）与 OpenRouter 消耗/限额，可手动刷新；无公开查询接口的提供方不展示
- **全局搜索与消息级定位** —— 首页搜索覆盖工作区与全部会话内容，命中按工作区分组并显示真实会话标题；点击直达并滚动高亮到匹配消息（深链 `?session=&seq=`）。中文短词由有界子串回填兜底（SQLite FTS 把连续中文当作一个 token）
- **轮次导航（长会话回跳）** —— 右下角浮出把手显示「当前 / 总轮」，向上翻历史时出现：轻点打开轮次清单（每轮带提问与回复预览，未加载的轮次会先分页再落位），按住左右拖动即刮擦条，松手直接跳到该轮；清单覆盖整个会话（实测 172 轮也能落到第 1 轮）
...
- **全局运行总览** —— 全屏查看 host 当前在做的一切：跨工作区的运行中会话与实时后台任务，按「正在运行 / 最近结束」分组；首页快捷 chip 带实时徽标，点开进入总览，点卡片直达对应会话
- **聊天内文件与图片预览** —— 消息里的文件路径链接（裸路径或反引号包裹，现均可点击）打开底部预览面板：markdown 格式化渲染、HTML 在沙箱 iframe 中渲染、代码高亮、图片内嵌显示；消息中的 `<img>` 图片支持全屏 lightbox（双击放大）
...
- **轮次导航** —— 右下角浮出把手显示「当前 / 总轮」，离开最新消息后就出现：轻点开轮次清单（每轮提问 + 回复预览，未加载的轮次先分页再落位），按住把手左右拖动＝横向刮擦条，松手跳到手指所指的那一轮。清单把宿主的 `turnOutline` 投影与本地已加载轮次合并（去重、按轮次升序），整段会话都可达；刮擦条按手指的绝对位置映射（拖到屏幕两端即最早/最新），深轮跳转按「下一轮的边界」取页并带页数预算，取不到会明确提示而非停在「正在载入…」
- **待发消息队列 dock** —— turn 运行期间发送的消息进入 host 队列，在输入区上方显示队列条（桌面 QueueDock 对齐）：单条或可折叠的「N 条排队」头；每条可编辑（仅纯文本）、删除、或插话发送（agent 运行中）；dock 镜像 host 快照，会话停止后自动消失
...
- **分类型通知门控** —— 规划完成 / 后台任务 / 长回复三类事件在设置页各有独立开关；存量配置默认后台任务与长回复安静（行为变更）
- **应用内系统通知（SSE）** —— 应用打开时手机端弹出系统通知；点击直达对应会话
- **通知收件箱** —— 通知页列出宿主引擎最近 50 条判定记录，错过即丢的通知可回看并点击直达所属会话
- **锁屏隐私** —— 开启后所有通道的通知统一降级为通用文案（不携带会话标题与任务名）
- **Web Push（VAPID）** —— 应用关闭时由服务工作者接收推送（Android Chrome/Edge/Firefox；iOS Safari 16.4+ 仅限已安装的 PWA；国内网络可能无法到达 FCM 后端——由第三方渠道覆盖）
- **第三方渠道** —— WxPusher（微信，国内用户推荐首选）、Server酱（微信）、Bark（iOS）、Telegram webhook，应用完全关闭也能送达；PushPlus 降级保留（现需付费认证，旧 Token 仍可发）。凭据只存宿主端，不经过设置面
- **通知设置** —— 权限、时长阈值、冷却间隔、Web Push 开关与渠道凭据都在手机端设置页，附「发送测试」按钮可端到端验证整条链路；各渠道显示凭据状态（已配置 / 未配置），重启后不再误以为 key 丢失
...
- **检查更新** —— 关于页代查 npm registry（手机端 CSP 禁止直连，由宿主代查），提示「发现新版本」或「已是最新」
...
- **首次上手欢迎卡** —— 配对成功后工作区首页展示一次性三步引导（开始对话 / 配通知 / 看用量），可关闭且仅显示一次
...
| L3 | WxPusher / Server酱 / Bark / Telegram / PushPlus（降级）webhook | 应用关闭、浏览器已退出 | 凭据只存宿主端 |
...
**L3（第三方）。** WxPusher（微信）、Server酱（微信）、Bark（iOS）、Telegram webhook 在应用完全关闭、浏览器退出后仍能送达；PushPlus 降级保留（现需付费认证）。凭据只存宿主端，设置面只显示各渠道是否已配置。
...
| 平台 | 安装 PWA | 说明 |
|---|---|---|
| Android | ✅ Chrome / Edge / Firefox | 浏览器菜单「安装应用」（需受信 HTTPS 入口） |
| iOS | ✅ 仅 Safari | 用 Safari「添加到主屏幕」；Chrome 等 iOS 浏览器不支持完整 PWA 安装——请用 Safari 打开 `/m/` |

iOS Web Push（16.4+）同样要求 Safari 安装的 PWA；Android 的推送后端（FCM）在国内网络可能不可达——请使用第三方渠道（L3）。
