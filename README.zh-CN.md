...
| | |
|---|---|
| ![聊天](docs/screenshots/chat.png) | **聊天** —— 流式 Markdown，代码块带语法高亮与行号 |
...
- **SSE mux 桥 + 轮询回退** —— 停滞检测（活跃流 36s）自动启动自适应历史轮询；事件不丢，SSE 恢复后立即切回
- **窗口化流式渲染** —— 只渲染可见前缀，长回复保持流畅
- **行内 Markdown 尾部预览** —— 流式尾部实时渲染 Markdown，无打字机光标
- **推理块折叠** —— 正文中的思考内容折叠为可展开块

**交互**

- **Diff 卡片** —— 容忍模型退化形状的 diff 解析 + 词级 LCS 标记，支持接受 / 拒绝 / 评审，多文件编辑在调用时间点合并
...
- **语音输入与转写** —— 浏览器内 WAV 录音，OpenAI 兼容多服务按序回退（默认 SenseVoiceSmall），手机端可管理服务列表
- **插件市场** —— 手机端浏览、搜索、安装插件

**离线与弱网**

- **PWA** —— 可安装、带版本号的服务工作者、可离线使用的打包产物
- **离线发件箱** —— 断网时消息排队（IndexedDB + 内存回退），联网自动发送，不丢不重
- **gzip 压缩 API 响应** —— 弱网友好
...
- **干净的流式输出** —— 窗口化前缀渲染、行内 Markdown 尾部预览、无打字机光标
- **推理块折叠** —— 正文中的思考内容折叠为可展开块
...
- **插件市场** —— 手机端浏览、搜索、安装插件
- **PWA** —— 可安装、带版本号的服务工作者、可离线使用的打包产物
- **离线发件箱** —— 断网时消息排队（IndexedDB），联网自动发送
- **gzip 压缩 API 响应** —— 弱网友好
- **按需加载** —— 工作区列表轻量拉取；会话只在打开时加载
- **桌面级设置同步** —— 手机端设置与桌面同步（schema 表单、级联模型选择、权限预设）
- **显示偏好** —— 字号、密度、行号、自动滚动
...
设备信任通过扫码配对建立；每次 `/m/api` 调用都由配对 cookie 与方法白名单把关。详见配对与 gate 源码，以及 [SECURITY.md](SECURITY.md) 的漏洞报告流程与部署注意事项。
