...
describe('/api/pair routes', () => {
  it('runs the full flow: issue (loopback) → accept (LAN) → cookie → reuse refused', async () => {
    ...
    try {
      ...
      expect(accepted.cookies).toEqual([
        'dsh_pair=tok-1; Path=/; HttpOnly; SameSite=Lax; Max-Age=7776000',
      ])
      // The same token is one-time: reuse is refused with the same uniform
      // response as an invalid token (no token-validity oracle for probes).
      const reused = await call(port, 'POST', '/api/pair/accept', { host: '192.168.1.5:3080', body: { token: 'tok-1' } })
      expect(reused.status).toBe(404)
      expect(reused.body.code).toBe('invalid')
  ...
  it('redacts the pairing oracle fields from unpaired status callers', async () => {
    ...
    try {
      ...
      // LAN literals reveal the host's network topology: also behind the gate.
      expect(unpaired.body).not.toHaveProperty('lanAddresses')
      expect(unpaired.body).not.toHaveProperty('devices')
