...
export async function issuePair(workspaceId?: string, address?: string): Promise<IssueResponse> {
  ...
  if (!response.ok) {
    ...
    throw new Error(`dsh-palm: issue failed with ${String(response.status)}`)
  }
 ...
 * sets the device cookie; the page then reloads to boot with it. Bounded by
 * the same 15 s timeout as the mobile accept path — a hung server must not
 * leave the boot flow spinning.
...
export async function stopPair(): Promise<void> {
  ...
  if (!response.ok) throw new Error(`dsh-palm: stop failed with ${String(response.status)}`)
}
