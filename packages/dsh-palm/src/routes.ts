...
/** Cookie lifetime: 90 days; revoked sessions die at the gate regardless. A
 * shorter life bounds the blast radius of a stolen device cookie (the cookie
 * is a full-authority credential) — Secure is impossible here because LAN
 * pairing runs over plain HTTP. */
...
export function makeRoutes(deps: PairRoutesDeps): WebRoute[] {
  ...
  const handleStatus = async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
    ...
    // token expiry, public tunnel URL, counts, and the LAN literals
    // themselves are an oracle for targeting and stay behind a live device
    // cookie (LAN IPs reveal the host's network topology). The per-device
    // roster (ids are session credentials) is never returned here — only the
    // loopback events stream carries it to the desktop panel.
