// Self-maintained copy (dsh-palm is a standalone repository; the original
// generator lived in the upstream monorepo). Keep the shared contract: one
// strict bounded body reader, one lenient bounded body reader, one JSON
// object narrow, and one JSON writer.
/**
...
/** Bodies at or above this length are gzip-compressed when the client asks. */
 ...
 * override them. When `req` is passed and the client accepts gzip, bodies
 * at or above {@link GZIP_MIN_BYTES} are compressed (the mobile surface
 * ships whole-session listings and history pages at hundreds of KiB plain;
 * small bodies are not worth the cost). A write onto a connection the client
 * already dropped is swallowed (ERR_STREAM_DESTROYED): the response is
 * unobservable anyway, and letting it escape would surface as an unhandled
 * rejection from the calling handler's catch path.
 */
export function writeJson(
  ...
  try {
    ...
    if (req !== undefined && payload.length >= GZIP_MIN_BYTES && acceptsGzip(req)) {
      try {
        ...
        // Compression failed; fall through to the plain response.
      }
    }
    res.writeHead(status, { ...JSON_HEADERS, ...headers })
    res.end(payload)
  } catch {
    // The client is gone; there is nobody to receive this response.
  }
}

/** Whether the request's Accept-Encoding lists gzip. */
function acceptsGzip(req: IncomingMessage): boolean {
  const header = req.headers['accept-encoding']
  if (typeof header !== 'string') return false
  return header.split(',').some(part => part.trim().split(';')[0] === 'gzip')
}
