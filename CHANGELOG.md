...
- **Long sessions are navigable on the phone again.** A floating handle in the bottom-right corner reports 当前 / 总轮 (the turn being read, out of the whole session) as soon as the reader leaves the newest message. Tapping it opens a turn-list sheet — every turn with its prompt and reply preview — and pressing it slides a horizontal scrubber in above the composer: the finger's absolute x maps onto the list, so dragging to either screen edge reaches the oldest and the newest turn, and releasing jumps. The list merges the host's `turnOutline` projection with the locally loaded turns (deduplicated, ascending) instead of relying on whichever turns happen to be paged in.
- **Jumping into deep history actually lands.** A turn's outline `seq` is its `turn/start` EVENT seq, which fires before the turn's first message and therefore sits in the gap BETWEEN two rendered rows — matching a row by seq range alone never found it, so a deep jump loaded a page and then stayed put. `turnRowIndex` now prefers the row carrying the turn number (the seq range stays as the fallback for search hits), and the page cursor is the NEXT turn's boundary: a page ends at the requested `beforeSeq`, so asking for the turn's own boundary fetched everything *before* it and excluded the turn itself. Jumps carry a page budget and say 「没能跳到那一轮：历史太深了」 rather than sitting on 「正在载入…」. Verified on a paired phone — a 172-turn session reaches turn 1 from the scrubber.
- **The phone buzzes when something needs a glance.** 设置 → 通用 gains a 震动 switch (on by default), and the notify path taps `navigator.vibrate(15)` — only for events that actually raise a notification while the app is in the background, never while the page is visible. Unsupported browsers (iOS Safari ships no `navigator.vibrate`), a denied permission and a throwing call are all silently inert by design: haptics must never be able to break the notification path.
- **The chat remembers where you left off in a session.** The reading position is stored per session and restored when it is opened again; it is written once scrolling pauses for a second (a write per pixel would churn localStorage) and flushed on unmount, so a session switch still writes the OLD session's position. Reading the newest turn does not move the bookmark — it points at the mid-history spot the reader actually left from, which is the one 当前/总轮 reports.
- **Code and text can leave the app through the system share sheet.** `shareText` prefers `navigator.share` and degrades to the clipboard (已复制，可粘贴分享) where the API is missing or the share itself fails; the user merely cancelling the sheet (AbortError) is silent, because cancelling is not a failure.

### Fixed

- **A connected phone no longer silences the desktop's approval and question panels.** The host bridge intercepted both host waterfalls ahead of the desktop chain whenever a phone was online, so `api-remotes` never saw the request: the desktop panel never appeared at all, and a phone that was not looking at that session left the turn hanging until it was interrupted (measured: 1m55s of nothing, then ABORTED). The request now reaches **both** ends and the first answer wins — the bridge hands it downstream (the desktop) *and* broadcasts it to the phone, withdraws the desktop's pending event through the gateway's own cancellation path when the phone answers first, and retires the phone's panel and pending tracker with `question/resolved` / `approval/resolved` when the desktop answers first. A desktop chain with no client looking at that session answers `NO_PROVIDER` immediately; that is "the desktop has nothing to answer with", not the end of the race, so the phone's panel stays live and can still answer. Covered by the bridge suite (either order, the preserved turn signal, the desktop-gives-up case) and verified end to end on a paired phone: an answer from the phone drops the desktop's panel at once, an answer from the desktop retires the phone's panel without it coming back after leaving and re-entering the session, and a phone that is not connected leaves the desktop's panel working normally.
- **One closed phone stream no longer leaves the host believing a phone is online.** The SSE close handler decremented the subscriber count and stopped; only the revocation path refreshed the "a phone is connected" flag, so after a backgrounded app or a tunnel blip the bridge kept forwarding approvals and questions to a stream nobody read — for the rest of the process. The flag now tracks the live subscriptions, which is what decides between the dual-answer race and the desktop-only path.
- **The remaining horizontal overflow on phone surfaces is gone.** Reply and question cards, tool pills, settings rows, run-overview session rows and the turn list could each push the page wider than the viewport (long paths, unbreakable tokens); those containers now shrink and wrap instead of overflowing.
